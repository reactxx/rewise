...
Future<Msg> _toStemmCacheLang(DataMsg msg, InitMsg initPar) async {
  ...
  // TODO: get words fr stemming
  final List<String> imputTexts = null;
  ...
  //       toPars.ParsedBook.fromBuffer(fileSystem.parsed.readAsBytes(bookFn));
    final List<String> texts = Linq.distinct(imputTexts.map((w) {
          if (w.isNotEmpty) return null;
          final txt = Langs.netToLower(w);
          if (cache.words.containsKey(txt)) return null;
          return txt;
