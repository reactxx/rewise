...
void analyzeWords(FileInfo first, List<Tuple2<FileInfo, Word>> fileWords) {
  ...
  final lines = list.map((kv) =>
      '${kv.value}x: ${kv.key.toString()}');
  fileSystem.edits
      .writeAsLines('analyzeSources\\words\\${first.dataLang}.txt', lines);
}

AnalyzeResult analyzeWord(FileInfo file, Word word) {
  final myScript = Langs.nameToMeta[file.dataLang].scriptId, otherScript = file.otherLang=='' ? '' : Langs.nameToMeta[file.otherLang].scriptId;
  var ok = 0,
      nonLetter = 0,
      latin = 0,
      //letter = 0,
  ...
  for (final ch in word.text.codeUnits) {
    ...
    if (uni.script == myScript)
      ok++;
    else if (uni == otherScript)
      other++;
    else
      wrongLetter++;
  }
  ...
  AnalyzeResult percent(AnalyzeResult gt66, AnalyzeResult gt33, AnalyzeResult any) {
    final okNum = ok/len;
    if (okNum < 0.33) return gt66;
    if (okNum < 0.66) return gt33;
    return any;
  }
  if (ok > 0 && len == ok + nonLetter) return percent(AnalyzeResult.nonLetterGt66, AnalyzeResult.nonLetterGt33, AnalyzeResult.nonLetterAny);
  if (ok > 0 && len == ok + latin) return percent(AnalyzeResult.latinGt66, AnalyzeResult.latinGt33, AnalyzeResult.latinAny);
  if (ok==0) return AnalyzeResult.mix;
  return percent(AnalyzeResult.mixGt66, AnalyzeResult.mixGt33, AnalyzeResult.mixAny);
}
